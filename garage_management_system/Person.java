...
public class Person
{
	...
	private final StringProperty lastName = new SimpleStringProperty(this , 
		"lastName" , null);
	private final ObjectProperty<LocalDate> birthDate = new SimpleObjectProperty(this , 
		"birthDate" , null);


	// Keeps track of the last generated person id
	...
	// personid Property;
	public final int getPersonId() //personId getter
	{
		return personId.get();
	}
	...
	{
		// getReadOnlyProperty is autogenerated code
	...
	public boolean isValidBirthDate(LocalDate bdate , List<String>errorList){
		...
		// birthDate cant be in the future
		if(bdate.isAfter(LocalDate.now()))
		{
			errorList.add("birthdate must not be in the future");
			return false;
		}
		return true;
	}
	...
	public boolean isValidPerson(Person p , List<String> errorList)
	{
		...
		if(fn == null || fn.trim().length() == 0){
			errorList.add("firstName must contain a minimum of one char");
			isValid= false;
		}
		String ln = p.lastName.get();
		if(ln == null || ln.trim().length() == 0){
			errorList.add("lastName must contain a minimum of one char");
			isValid= false;
		}
	...
	public AgeCategory getAgeCategory()
	{
		...
		if(years >= 0 && years < 2){
			...
			// An enum is also called the same way as a method
		}
